...
class QueueManagementService {
  ...
  Future<void> checkStuckItems() async {
    ...
    final stuckItems = await _dbService.db.syncQueueEntitys
        .where()
  ...
  Future<void> deleteStuckItem(String itemId) async {
    await _dbService.db.writeTxn(() async {
      final item = _dbService.db.syncQueueEntitys.getByIdSync(itemId);
      if (item != null) {
        await _dbService.db.syncQueueEntitys.delete(item.isarId);
  ...
  Future<Map<String, int>> getPendingSummary() async {
    final items = await _dbService.db.syncQueueEntitys
        .where()
        .syncStatusEqualTo(SyncStatus.pending)
        .findAll();
